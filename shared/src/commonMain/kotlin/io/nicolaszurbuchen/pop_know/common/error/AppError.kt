...
sealed interface AppError {
    sealed interface Network : AppError {
        ...
        data class Http(val code: Int, val serverMessage: String? = null) : Network
    }

    sealed interface Database : AppError {
        data class QueryFailed(val cause: Throwable) : Database
        data class InsertFailed(val cause: Throwable) : Database
    }
